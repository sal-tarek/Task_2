...
export async function login(req, res, next) {
  try {
    ...
    const existing = await User.findOne({ email: value.email });
    if (!existing) return res.status(409).json({ message: 'Email not found' });

    const passwordCheck = await bcrypt.compare(value.password, existing.passwordHash);
    if(!passwordCheck) return res.status(409).json({ message: 'Incorrect' });
